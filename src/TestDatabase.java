...
public class TestDatabase {
	...
	private static void createDatabase() {
		try (
			...
			//create the database
			String database = "create database if not exists Test";
	...
	private static void createTable() {
		try (
			Connection conn = DriverManager.getConnection(
					...
					"/Test?user=root&password=root"); // MySQL

			Statement statement = conn.createStatement();
		) {
			//create a table of all the players
			String playerTable = "create table if not exists Test ( " +
				"Name varchar(50), " +
				"Image blob, " +
				"Sport varchar(50), " +
				"Gender varchar(50), " +
	...
	private static void addPlayersToDatabase() {
		try (
			...
			String insertPlayers = "insert ignore into Test (Name, Image, Sport, Gender, Height, GamesPlayed, TeamWins, TeamLosses, ClassYear) values "
					+ "('Joran Meltzer', LOAD_FILE('C:\\Users\\Nicole\\Desktop\\HH2Git\\220px-Cat-eating-prey.jpg'), 'Basketball', 'Female', 63, 25, 2, 23, 2);";
	...
	private static void getPlayersFromDatabase() {
		try (
			...
			String getEverything = "select Name, Sport, Gender, Height, GamesPlayed, TeamWins, TeamLosses, ClassYear from Test";
			...
			while (allPlayers.next()) {
				...
				//Blob image = allPlayers.getString("Image");
